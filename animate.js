...
function parseColor(color) {
    ...
    if (result) {
        ...
        if (h > 360 || h < 0 || s > 100 || s < 0 || l > 100 || l < 0 || a > 1 || a < 0) {
            throw new Error(`Invalid color value: ${color} hints: 0 <= h <= 360, 0% <= s, l <= 100%, 0 <= a <= 1`);
        }
...
const animate = function (partialOptions = {}, startState, endState, updateFunc) {
    ...
    function animation() {
        ...
        for (const key in startState) {
            if (key.toLowerCase().includes('color')) {
                ...
                currentValues[key] =
                    startState[key] +
                        progress * (endState[key] - startState[key]);
            }
        }
        ...
        if (rawProgress < 1 || elapsed < options.duration + options.endDelay) {
            ...
            if (options.iterations === 'Infinity' || (options === null || options === void 0 ? void 0 : options.iterations) > iterationCount) {
